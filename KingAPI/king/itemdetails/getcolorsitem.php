...
");
 ...
 for( $i=0;$i < $count;$i++){
    ...
    WHERE siz.id=cartclr.size_id and cartclr.clr_id=".$color["id"]." and cartclr.itm_id=$itmid  and siz.id=itm.size_id and 
    itm.itm_id=$itmid  and itm.clr_id=".$color["id"]." and cartclr.usr_id=$userid 
    and itm.size_id in 
    (select cartsiz.size_id from cartcolorsizes as cartsiz,cart where cartsiz.usr_id=$userid and cartsiz.clr_id=".$color["id"]."
    and cartsiz.itm_id=$itmid  and cartsiz.cart_id=cart.id and cart.order_id=0 and cart.status=0) 
    ...
    WHERE siz.id=itm.size_id and itm.itm_id=$itmid  and itm.clr_id=".$color["id"]."
    and itm.size_id not in 
    (select cartsiz.size_id from cartcolorsizes as cartsiz,cart where cartsiz.usr_id=$userid and cartsiz.clr_id=".$color["id"]."
    and cartsiz.itm_id=$itmid  and cartsiz.cart_id=cart.id and cart.order_id=0 and cart.status=0) 
